...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::ReturnToHydro { vessel_id } => execute_return_to_hydro(deps, info, vessel_id),
    }
}

fn execute_return_to_hydro(
    deps: DepsMut,
    info: MessageInfo,
    vessel_id: u64,
) -> Result<Response, ContractError> {
    ...
    validate_user_owns_vessels(deps.storage, &info.sender, &[vessel_id])?;
    ...
    let execute_transfer_nft_msg = WasmMsg::Execute {
    ...
    Ok(Response::new()
        ...
        .add_attribute("vessel_id", vessel_id.to_string())
